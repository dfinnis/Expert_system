...
unit_test()
{
	...
	then
		((passed+=1))
		printf "$Green%-25s OK$Reset\n" $Filename
	else
		printf "$Red%-25s ERROR$Reset\n" $Filename
...
unit_test input/invalid/ "Error: Invalid filepath"
...
#### -- TOTAL -- ####
echo "\n\n$Bright All Expert System tests finished\nTotal runtime $SECONDS seconds$Reset"
